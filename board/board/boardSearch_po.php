...
<?php
    ...
    //쿼리문(JOIN)
    //b.myBoardID, b.boardTitle, b.boaedContents, m.youName, b.regTime, b.boardView
    ...
    switch($searchOption){
        case 'title';
            ...
            break;
        case 'content';
            $sql .= "WHERE b.boardContents LIKE '%{$searchKeyword}%' ORDER BY myBoardID";
            break;
        case 'name';
            $sql .= "WHERE m.youName LIKE '%{$searchKeyword}%' ORDER BY myBoardID";
            break;
    }
...
if($page != $boardCount){
    $nextPage = $page + 1;
    echo "<li><a href='boardSearch.php?page={$nextPage}'>다음</a></li>";
    echo "<li><a href='boardSearch.php?page={$boardCount}'>마지막으로</a></li>";
}
?>
                    </ul>
                </div>
